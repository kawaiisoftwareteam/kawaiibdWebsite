...
const Seminar = () => {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText('https://kawaiibd.com/seminar');
    setCopied(true);
  ...
  const validateStep = (step) => {
    const newErrors = {};
    if (step === 1) {
      if (!formData.name.trim()) newErrors.name = t('seminar.form.errors.nameRequired');
      if (!formData.email.trim()) {
        newErrors.email = t('seminar.form.errors.emailRequired');
      } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
        newErrors.email = t('seminar.form.errors.emailInvalid');
      }
      if (!formData.phone.trim()) newErrors.phone = t('seminar.form.errors.phoneRequired');
    } else if (step === 2) {
      if (!formData.status) newErrors.status = t('seminar.form.errors.statusRequired');
      if (!formData.org.trim()) newErrors.org = t('seminar.form.errors.orgRequired');
      if (!formData.designation.trim()) newErrors.designation = t('seminar.form.errors.designationRequired');
      if (!formData.experience) newErrors.experience = t('seminar.form.errors.experienceRequired');
    } else if (step === 3) {
      if (!formData.familiarity) newErrors.familiarity = t('seminar.form.errors.familiarityRequired');
      if (!formData.opinion.trim()) newErrors.opinion = t('seminar.form.errors.opinionRequired');
      if (!formData.bimLevel) newErrors.bimLevel = t('seminar.form.errors.bimLevelRequired');
    } else if (step === 4) {
      if (!formData.source) newErrors.source = t('seminar.form.errors.sourceRequired');
    }
    setErrors(newErrors);
  ...
  const GOOGLE_FORM_ACTION =
    'https://docs.google.com/forms/d/e/1FAIpQLScG5Des9dhP7P1nN80Ro0ejqeedRx1fMV7WdtMbnd9OcbL-WA/formResponse';
        ...
        >
          ...
          >
            <button
              ...
              onClick={resetForm}
              aria-label={t('seminar.form.close')}
            >
              ×
            </button>
            ...
            <div className="space-y-2 text-center">
              <h3 id="registration-success-title" className="text-2xl sm:text-3xl font-extrabold text-gray-900">
                {t('seminar.success.title')}
              </h3>
              <p className="text-base sm:text-lg text-gray-600 max-w-md mx-auto">
                {t('seminar.success.thankYouPrefix')}{' '}
                <span className="font-semibold text-gray-900">{formData.name}</span>.{' '}
                {t('seminar.success.thankYouSuffix')}
              </p>
            </div>
            <div className="bg-gray-50 border border-gray-200 p-4 sm:p-5 rounded-2xl text-left space-y-3 w-full">
              <h4 className="font-bold text-gray-900 text-base flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-rose-600"></span> {t('seminar.success.nextTitle')}
              </h4>
              <ul className="text-sm text-gray-600 space-y-2 list-disc pl-5">
                <li>{t('seminar.success.next1')}</li>
                <li>{t('seminar.success.next2')}</li>
                <li>{t('seminar.success.next3')}</li>
              </ul>
            </div>
            <button
              type="button"
  ...
  return (
    <div className="seminar-container min-h-screen pb-4 sm:pb-8">
      ...
      <div className="seminar-page-shell mb-8 sm:mb-12 bg-gradient-to-br from-gray-950 via-gray-900 to-slate-900 text-white rounded-3xl p-6 sm:p-10 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 border border-gray-800/80 relative overflow-hidden">
        ...
        <div className="space-y-2 text-center md:text-left w-full md:w-auto z-10">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold bg-rose-500/20 text-rose-300 border border-rose-500/30 uppercase tracking-widest pulse-animation">
            ...
            {t('seminar.urgency.badge')}
          </span>
          <h2 className="text-2xl sm:text-4xl font-black tracking-tight text-white">{t('seminar.urgency.title')}</h2>
          <p className="text-sm sm:text-base text-gray-300 font-medium">{t('seminar.urgency.dateLabel')}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 w-full md:w-auto z-10">
          <div className="flex gap-2 sm:gap-3 text-center w-full sm:w-auto justify-between sm:justify-center">
            <div className="countdown-unit bg-gray-900/90 rounded-2xl border border-gray-700/70 p-3 sm:p-4 min-w-[70px] sm:min-w-[84px] flex-1 sm:flex-none shadow-inner">
              ...
              <span className="text-[10px] sm:text-xs text-gray-400 uppercase font-bold tracking-wider">{t('seminar.countdown.days')}</span>
            </div>
            <div className="countdown-unit bg-gray-900/90 rounded-2xl border border-gray-700/70 p-3 sm:p-4 min-w-[70px] sm:min-w-[84px] flex-1 sm:flex-none shadow-inner">
              <span className="countdown-value block font-black text-2xl sm:text-3xl text-rose-400">{timeLeft.hours}</span>
              <span className="text-[10px] sm:text-xs text-gray-400 uppercase font-bold tracking-wider">{t('seminar.countdown.hours')}</span>
            </div>
            <div className="countdown-unit bg-gray-900/90 rounded-2xl border border-gray-700/70 p-3 sm:p-4 min-w-[70px] sm:min-w-[84px] flex-1 sm:flex-none shadow-inner">
              <span className="countdown-value block font-black text-2xl sm:text-3xl text-rose-400">{timeLeft.minutes}</span>
              <span className="text-[10px] sm:text-xs text-gray-400 uppercase font-bold tracking-wider">{t('seminar.countdown.mins')}</span>
            </div>
            <div className="countdown-unit bg-gray-900/90 rounded-2xl border border-gray-700/70 p-3 sm:p-4 min-w-[70px] sm:min-w-[84px] flex-1 sm:flex-none shadow-inner">
      ...
      <div className="seminar-page-shell grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(320px,380px)] xl:grid-cols-[minmax(0,1fr)_minmax(340px,400px)] gap-8 sm:gap-10 items-start mb-16">
        ...
        <div className="space-y-8 sm:space-y-10 min-w-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 flex items-start gap-4 hover-card-effect relative overflow-hidden group">
              ...
              <div className="min-w-0">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('seminar.meta.date.label')}</h4>
                <p className="text-lg sm:text-2xl font-black text-slate-900 mt-1">{t('seminar.meta.date.value')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium mt-1">{t('seminar.meta.date.note')}</p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 flex items-start gap-4 hover-card-effect relative overflow-hidden group">
              ...
              <div className="min-w-0">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('seminar.meta.time.label')}</h4>
                <p className="text-lg sm:text-2xl font-black text-slate-900 mt-1">{t('seminar.meta.time.value')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium mt-1">{t('seminar.meta.time.note')}</p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 flex items-start gap-4 hover-card-effect relative overflow-hidden group">
              ...
              <div className="min-w-0">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('seminar.meta.location.label')}</h4>
                <p className="text-lg sm:text-2xl font-black text-slate-900 mt-1">{t('seminar.meta.location.value')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium mt-1">{t('seminar.meta.location.note')}</p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 flex items-start gap-4 hover-card-effect relative overflow-hidden group">
              ...
              <div className="min-w-0">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('seminar.meta.incentives.label')}</h4>
                <p className="text-lg sm:text-2xl font-black text-slate-900 mt-1">{t('seminar.meta.incentives.value')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium mt-1">{t('seminar.meta.incentives.note')}</p>
              </div>
            </div>

            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 flex items-start gap-4 hover-card-effect sm:col-span-2 relative overflow-hidden group">
              <div className="p-4 bg-gradient-to-br from-rose-50 to-rose-100/80 text-[#be1e2d] rounded-2xl flex-shrink-0 shadow-inner border border-rose-100/80 group-hover:scale-105 transition-transform duration-300">
                <FaInfoCircle className="text-2xl sm:text-3xl" />
              </div>
              <div className="min-w-0">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('seminar.meta.deadline.label')}</h4>
                <p className="text-lg sm:text-2xl font-black text-slate-900 mt-1">{t('seminar.meta.deadline.value')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium mt-1">{t('seminar.meta.deadline.note')}</p>
              </div>
            </div>
          </div>
          ...
          <div id="speaker-section" className="bg-white p-6 sm:p-10 rounded-3xl shadow-sm border border-slate-200/80 relative overflow-hidden hover-card-effect">
            ...
            <div className="flex flex-col md:flex-row items-start gap-6 sm:gap-10">
              ...
              <div className="w-full md:w-52 flex flex-col items-center gap-4 flex-shrink-0">
                ...
                <div className="w-full grid grid-cols-2 gap-2.5 bg-slate-50/90 p-3.5 rounded-2xl border border-slate-200/60 text-center shadow-inner">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 uppercase font-extrabold">{t('seminar.speaker.stats.gcRd')}</div>
                  </div>
                  <div>
                    <div className="text-xl font-black text-slate-900">20+</div>
                    <div className="text-[10px] text-slate-400 uppercase font-extrabold">{t('seminar.speaker.stats.papers')}</div>
                  </div>
                  <div>
                    <div className="text-xl font-black text-slate-900">10</div>
                    <div className="text-[10px] text-slate-400 uppercase font-extrabold">{t('seminar.speaker.stats.patents')}</div>
                  </div>
                  <div>
                    <div className="text-xl font-black text-slate-900">2</div>
                    <div className="text-[10px] text-slate-400 uppercase font-extrabold">{t('seminar.speaker.stats.committees')}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-5 flex-1">
                <div>
                  <h3 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">{t('seminar.speaker.name')}</h3>
                  <p className="text-lg font-bold text-[#be1e2d] mt-1">{t('seminar.speaker.role')}</p>
                  <p className="text-sm font-semibold text-slate-500 mt-0.5">{t('seminar.speaker.company')}</p>
                </div>
                
                <p className="text-lg text-slate-600 leading-relaxed font-normal">
                  {t('seminar.speaker.bio')}
                </p>

                <div className="space-y-3 border-t border-slate-100 pt-4">
                  <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-widest">{t('seminar.speaker.timelineTitle')}</h4>
          ...
          <div id="chief-guest-section" className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200/80 relative overflow-hidden hover-card-effect">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8">
              ...
              <div className="space-y-2 text-center sm:text-left flex-1">
                <h3 className="text-2xl sm:text-3xl font-black text-slate-900">{t('seminar.chiefGuest.name')}</h3>
                <p className="text-lg font-bold text-[#be1e2d]">{t('seminar.chiefGuest.role')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium">{t('seminar.chiefGuest.company')}</p>
              </div>
            </div>
          </div>
          ...
          <div id="special-guest-section" className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200/80 relative overflow-hidden hover-card-effect">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8">
              ...
              <div className="space-y-2 text-center sm:text-left flex-1">
                <h3 className="text-2xl sm:text-3xl font-black text-slate-900">{t('seminar.specialGuest.name')}</h3>
                <p className="text-lg font-bold text-[#be1e2d]">{t('seminar.specialGuest.role')}</p>
                <p className="text-sm sm:text-base text-slate-500 font-medium">{t('seminar.specialGuest.company')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 sm:p-10 rounded-3xl shadow-sm border border-slate-200/80 space-y-6 hover-card-effect">
            <h3 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{t('seminar.modules.title')}</h3>
        ...
        <div className="space-y-8 lg:sticky lg:top-24">
          <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200/80 sharing-card space-y-6 flex flex-col items-center hover-card-effect">
            <div className="text-center">
              <h3 className="font-black text-slate-900 text-xl tracking-tight">{t('seminar.share.title')}</h3>
              <p className="text-base text-slate-500 font-medium mt-2">{t('seminar.share.desc')}</p>
            </div>
            ...
            <div className="w-full space-y-3 pt-2">
              ...
              >
                {copied ? <><FaCheckCircle className="text-emerald-500 text-lg" /> {t('seminar.share.linkCopied')}</> : <><FaCopy className="text-slate-500" /> {t('seminar.share.copyLink')}</>}
              </button>
              <a
                href={`mailto:?subject=${encodeURIComponent(t('seminar.share.emailSubject'))}&body=${encodeURIComponent(`${t('seminar.share.emailBody')} ${registrationLink}`)}`}
              ...
              >
                <FaShareAlt /> {t('seminar.share.shareEmail')}
              </a>
            </div>
          </div>
          ...
          <div className="bg-rose-50/70 p-6 rounded-3xl border border-rose-200/60 flex items-start gap-4 shadow-sm">
            ...
            <div className="text-sm text-rose-900 space-y-1.5 leading-relaxed font-medium">
              <p className="font-extrabold text-base">{t('seminar.help.title')}</p>
              <p>{t('seminar.help.text')} <a href="mailto:info@kawaiibd.com" className="font-bold underline text-[#be1e2d]">info@kawaiibd.com</a>.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="register-section" className="seminar-form-shell bg-white rounded-2xl sm:rounded-3xl shadow-xl border border-gray-150 overflow-hidden mb-10 sm:mb-16">
        ...
        <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white p-5 sm:p-10 relative">
          ...
          <h2 className="text-xl sm:text-4xl font-extrabold text-white pr-8">{t('seminar.form.title')}</h2>
          <p className="text-gray-300 text-sm sm:text-lg mt-2 max-w-xl">
            {t('seminar.form.subtitle')}
          </p>
        </div>
          ...
          <div className="p-4 sm:p-10">
              ...
              {[1, 2, 3, 4].map((stepNum) => (
                <div key={stepNum} className="flex items-center flex-1 last:flex-initial">
                  <button
                    ...
                    aria-label={`${t('seminar.form.stepLabel')} ${stepNum}`}
                  >
            ...
            >
              ...
              <div className={currentStep === 1 ? "space-y-5 sm:space-y-6" : "hidden"}>
                <div className="border-b border-gray-100 pb-3">
                  <h3 className="font-black text-gray-900 flex items-start sm:items-center gap-2 flex-wrap">
                    <FaUser className="text-[#be1e2d] mt-1 sm:mt-0 flex-shrink-0" /> {t('seminar.form.steps.s1')}
                  </h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-2">
                    <label htmlFor="name" className="block font-black text-gray-900">{t('seminar.form.fields.fullName')}</label>
                    <input
                      id="name"
                      ...
                      placeholder={t('seminar.form.placeholders.name')}
                      autoComplete="name"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="email" className="block font-black text-gray-900">{t('seminar.form.fields.email')}</label>
                    <input
                      id="email"
                      ...
                      placeholder={t('seminar.form.placeholders.email')}
                      autoComplete="email"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="phone" className="block font-black text-gray-900">{t('seminar.form.fields.phone')}</label>
                    <input
                      id="phone"
                      ...
                      placeholder={t('seminar.form.placeholders.phone')}
                      autoComplete="tel"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="linkedin" className="block font-black text-gray-900">{t('seminar.form.fields.linkedin')}</label>
                    <input
                      id="linkedin"
                      ...
                      placeholder={t('seminar.form.placeholders.linkedin')}
              ...
              <div className={currentStep === 2 ? "space-y-5 sm:space-y-6" : "hidden"}>
                <div className="border-b border-gray-100 pb-3">
                  <h3 className="font-black text-gray-900 flex items-start sm:items-center gap-2 flex-wrap">
                    <FaBriefcase className="text-[#be1e2d] mt-1 sm:mt-0 flex-shrink-0" /> {t('seminar.form.steps.s2')}
                  </h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-2">
                    <label htmlFor="status" className="block font-black text-gray-900">{t('seminar.form.fields.status')}</label>
                    <select
                      id="status"
                    ...
                    >
                      <option value="">{t('seminar.form.selectStatus')}</option>
                      {STATUS_KEYS.map((key) => (
                        <option key={key} value={FORM_VALUES.status[key]}>
                          {t(`seminar.form.statusLabels.${key}`)}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="space-y-2">
                    <label htmlFor="org" className="block font-black text-gray-900">{t('seminar.form.fields.org')}</label>
                    <input
                      id="org"
                      ...
                      placeholder={t('seminar.form.placeholders.org')}
                  ...
                  <div className="space-y-2">
                    <label htmlFor="designation" className="block font-black text-gray-900">{t('seminar.form.fields.designation')}</label>
                    <input
                      id="designation"
                      ...
                      placeholder={t('seminar.form.placeholders.designation')}
                  ...
                  <div className="space-y-2">
                    <label htmlFor="experience" className="block font-black text-gray-900">{t('seminar.form.fields.experience')}</label>
                    <select
                      id="experience"
                    ...
                    >
                      <option value="">{t('seminar.form.selectExperience')}</option>
                      {EXPERIENCE_KEYS.map((key) => (
                        <option key={key} value={FORM_VALUES.experience[key]}>
                          {t(`seminar.form.experienceLabels.${key}`)}
                        </option>
                      ))}
                    </select>
              ...
              <div className={currentStep === 3 ? "space-y-5 sm:space-y-6" : "hidden"}>
                <div className="border-b border-gray-100 pb-3">
                  <h3 className="font-black text-gray-900 flex items-start sm:items-center gap-2 flex-wrap">
                    <FaGraduationCap className="text-[#be1e2d] mt-1 sm:mt-0 flex-shrink-0" /> {t('seminar.form.steps.s3')}
                  </h3>
                </div>
                ...
                <div className="space-y-3">
                  <label className="block font-black text-gray-900">
                    {t('seminar.form.fields.familiarity')}
                  </label>
                    ...
                    {FAMILIARITY_KEYS.map((key) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          <span className="font-bold text-gray-800">{t(`seminar.form.familiarityLabels.${key}`)}</span>
                        </label>
                      );
                    })}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder={t('seminar.form.placeholders.opinion')}
                ...
                <div className="space-y-3">
                  <label className="block font-black text-gray-900">
                    {t('seminar.form.fields.bimLevel')}
                  </label>
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            className="h-6 w-6 text-[#be1e2d] border-gray-300 focus:ring-rose-500"
                          />
                          <span className="font-bold text-gray-800">{t(`seminar.form.bimLevelLabels.${key}`)}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className={currentStep === 4 ? "space-y-5 sm:space-y-6" : "hidden"}>
                <div className="border-b border-gray-100 pb-3">
                  <h3 className="font-extrabold text-gray-900 flex items-start sm:items-center gap-2 flex-wrap">
                    <FaAward className="text-[#be1e2d] mt-1 sm:mt-0 flex-shrink-0" /> {t('seminar.form.steps.s4')}
                  </h3>
                </div>
                    ...
                    {GOAL_KEYS.map((key) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          <span className="font-bold text-gray-800 leading-snug">{t(`seminar.form.goalsLabels.${key}`)}</span>
                        </label>
                      );
                    })}
                  </div>
                ...
                <div className="space-y-4">
                  <label className="block font-black text-gray-900">
                    {t('seminar.form.fields.autocad')}
                  </label>
                  <div className="flex items-center gap-2 sm:gap-4">
                    <span className="text-[10px] sm:text-sm text-gray-400 font-bold uppercase flex-shrink-0">{t('seminar.form.autocadBeginner')}</span>
                    ...
                    <span className="text-[10px] sm:text-sm text-gray-400 font-bold uppercase flex-shrink-0">{t('seminar.form.autocadAdvanced')}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label htmlFor="suggestions" className="block font-black text-gray-900">
                    {t('seminar.form.fields.suggestions')}
                  </label>
                  <textarea
                    id="suggestions"
                    ...
                    placeholder={t('seminar.form.placeholders.suggestions')}
                    className="w-full rounded-2xl border border-gray-300 font-semibold form-input"
                  ></textarea>
                </div>
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            className="h-6 w-6 text-[#be1e2d] border-gray-300 focus:ring-rose-500"
                          />
                          <span className="font-bold text-gray-800">{t(`seminar.form.sourceLabels.${key}`)}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="form-nav-actions pt-5 sm:pt-6 border-t border-gray-100">
                {currentStep > 1 ? (
                  ...
                  >
                    <FaArrowLeft /> {t('seminar.form.prev')}
                  </button>
                ) : (
                  <div></div>
                )}
                ...
                {currentStep < 4 ? (
                  ...
                  >
                    {t('seminar.form.next')} <FaArrowRight />
                  </button>
                ) : (
                  <button
                    type="submit"
                  ...
                  >
                    {t('seminar.form.submit')} <FaCheckCircle />
                  </button>
                )}
              </div>
            </form>
